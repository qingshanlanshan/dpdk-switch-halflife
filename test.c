    ...
    for (i = 0; !force_quit; i = (i + 1) % app.n_ports)
    {
        ...
        if (now_time - last_output_time[i] > output_gap)
        {
            ...
            if (ret == 0)
            {
                ...
                RTE_LOG(INFO, SWITCH, "Time: %-5fs Port %d: ipkts=%-10ld  opkts=%-10ld  irate=%-10fMbps orate=%-10fMbps n_flowlet=%ld n_fw=%ld tot_cyc=%ld cyc_per_pkt=%f\n",
                        time_in_s, i, port_stats.ipackets, port_stats.opackets, irate, orate, app.flowlet_counter,app.n_fw,app.cyc,app.cyc*1.0/app.n_fw);
